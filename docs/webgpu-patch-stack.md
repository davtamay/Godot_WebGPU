...
| 35 | [shared] xr: Draw XR viewports one pass per view without multiview | WGSL has no multiview, so stereo on the WebGPU backend renders the viewport once per view: XRInterface gains get_draw_pass_count()/get_current_draw_pass()/set_current_draw_pass() (defaults keep every other interface and platform on the existing single-pass path) and the viewport's XR block loops them. Upstream's multi-layer projection camera (godotengine 9d0b0a6738, merged 2026-09-10) computes every view's projection and offset on the main thread and stores them on the Camera, so render_camera picks the active pass's entry and renders it as a single-view camera; the WebXR interface reports ONE view to the renderer while its camera projections/offsets still carry every WebXR view, and hands each pass its view's layer slice via texture_create_shared_from_slice. Flips ENGINE_WEBXR_WEBGPU_SUPPORTED: XR-flagged exports now genuinely boot WebGPU on XRGPUBinding browsers | ~9 xr_interface.h, ~12 renderer_viewport.cpp, ~20 renderer_scene_cull.cpp, ~4 engine.js, rest in modules/webxr | not yet |
