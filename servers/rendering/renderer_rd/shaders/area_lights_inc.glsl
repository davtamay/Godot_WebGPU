...
// The two LTC tables are layers of one array texture: same 64x64 size and
// format, and one binding instead of two, which matters where the per-stage
// texture budget is tight (WebGPU's floor is 16).
void ltc_evaluate_specular(vec3 normal, vec3 eye_vec, float roughness, vec3 points[4], vec4 texture_rect, float max_mipmap, texture2D area_light_atlas, sampler texture_sampler, sampler lut_sampler, texture2DArray ltc_luts, out float ltc_specular, out vec2 fresnel, out vec3 ltc_specular_tex_color) {
	...
	vec4 M_brdf_abcd = texture(sampler2DArray(ltc_luts, lut_sampler), vec3(lut_uv, 0.0));
	vec3 M_brdf_e_mag_fres = texture(sampler2DArray(ltc_luts, lut_sampler), vec3(lut_uv, 1.0)).xyz;
